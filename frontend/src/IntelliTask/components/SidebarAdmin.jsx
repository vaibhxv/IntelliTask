...
export const SidebarAdmin = () => {
  ...
  return (
    <>
      ...
      <aside ref={sidebar} className="fixed z-10 top-0 pt-32 md:pt-20 left-0 w-64 h-screen bg-gradient-to-b from-gray-50 to-gray-400 transition-transform duration-500 -translate-x-full md:translate-x-0 shadow" aria-label="Sidebar">
        <div className="h-full px-6 py-6 overflow-y-auto">
          <ul className="space-y-2 font-medium">
            <li>
              <NavLink
                className={({ isActive }) => `${isActive ? 'bg-gray-800 text-white' : ''} flex items-center gap-2 p-2 rounded-lg hover:bg-gray-800 text-[#545454] hover:text-white transition-colors w-full`}
                to="/dashboard"
          ...
          <ul className="pt-4 mt-4 space-y-2 font-medium border-t border-gray-400">
            <li>
              <button
                type='button'
                className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-800 text-[#545454] hover:text-white transition-colors w-full"
                onClick={onLogout}
              >
                <LogoutIcon />
                Log Out
